...
One of the common misconceptions is that serverless is only for simple code functions, major cloud providers now support running stateless containers in a serverless fashion as well.
